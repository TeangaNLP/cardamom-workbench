...
def pretrained_embedding_matrix(file_name):
    ...
    inputs,raw_inputs, raw_seqs,targets = prepare_sentence(f'{slash}home{slash}kougos{slash}data{slash}language_detection_dataset{slash}supervised_code_model{slash}fasttext_k-means{slash}train_full_3k.csv')
...
def prepare_dataset(train_file,batch_size,eval_batch_size):
        ...
        file_name = open(f'{slash}home{slash}kougos{slash}data{slash}language_detection_dataset{slash}fasttext_k-means{slash}embedding_final.vec', 'r', encoding='utf8', errors='ignore')
