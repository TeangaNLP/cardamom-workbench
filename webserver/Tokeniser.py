...
def cardamom_tokenise(string, provenance=None, iso_code=None, reserved_toks=None):
    ...
    if reserved_toks:
        ...
        for indset in reserved_toks:
            if not isinstance(indset, dict):
                raise RuntimeError(f"\nExpected array of reserved tokens where each token is of type 'dict',\neg. "
                                   f"[{{'type': 'auto', 'start_index': 0, 'end': 5, 'text_language': 'en',"
                                   f"'token_language': 'en', 'provenance': 1}}, {{...}}, ...]\n"
        ...
        for indset in reserved_toks:
            ...
            checklist = ["type", "start_index", "end", "provenance"]
            ...
            # Check that values of start_index and end indices for reserved tokens are integers. If not, raise error.
            if not all(isinstance(index, int) for index in [start_index, end]):
            ...
            if start_index < current_index:
                raise RuntimeError(f"Reserved tokens overlap or are not in order of occurrence.\nStarting index "
                                   f"({start_index}) of token, {indset}, precedes end index of preceding token "
                                   f"({current_index}).")
    ...
    for token in token_list:
        ...
        if isinstance(token, str):
            ...
            tok_dict = {"type": "auto", "start_index": tok_index, "end": tok_index + len(token),
...
#
#     res_toks = [{"type": "manual", "start_index": 51, "end": 60, "provenance": 2},
#                 {"type": "manual", "start_index": 153, "end": 163, "provenance": 2},
#                 {"type": "manual", "start_index": 165, "end": 171, "provenance": 3}]
#
